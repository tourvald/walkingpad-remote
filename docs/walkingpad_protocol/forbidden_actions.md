...
| Store operator visual physical-semantics confirmation | `allowed` | Evidence only; per-device fingerprint; does not unlock HR-control. | [ks_f0_case_notes.md](ks_f0_case_notes.md) |
...
| HR-control on imperial | `forbidden` for current product | Physical command semantics and stop safety are not fully resolved. | [units_and_setunit.md](units_and_setunit.md), [ks_f0_case_notes.md](ks_f0_case_notes.md) |
| Automatic mph/kmh command conversion for HR-control | `forbidden` | Would create safety-critical speed changes without approved model. | [units_and_setunit.md](units_and_setunit.md) |
