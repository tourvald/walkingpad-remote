...
private struct DebugView: View {
    ...
    private var unitsDiagnosticsSection: some View {
        ...
        DebugSectionCard(
            ...
            subtitle: "Dangerous debug: writes persistent controller preferences. Use only on test controller."
        ) {
            VStack(alignment: .leading, spacing: 14) {
    ...
    var body: some View {
        NavigationStack {
            ...
            .alert("Dangerous units debug?", isPresented: Binding(
                get: { pendingUnitsDangerousAction != nil },
                set: { isPresented in
                    if !isPresented { pendingUnitsDangerousAction = nil }
                }
            )) {
